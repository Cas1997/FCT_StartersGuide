...
// ********************************************************************
//
/// \file optrChangeCrossSection.cc
/// \brief Implementation of the OptrChangeCrossSection class
//
#include "optrChangeCrossSection.hh"
...
OptrChangeCrossSection::OptrChangeCrossSection(G4String particleName,
                                                         G4String         name)
  ...
  if ( fParticleToBias == 0 )
    {
      ...
      G4Exception("OptrChangeCrossSection(...)",
                  "ex01",
                  JustWarning,
                  ed);
    }
...
OptrChangeCrossSection::~OptrChangeCrossSection()
{
...
void OptrChangeCrossSection::StartRun()
{
  // --------------
  // -- Setup stage:
  // ---------------
...
G4VBiasingOperation* 
OptrChangeCrossSection::ProposeOccurenceBiasingOperation(const G4Track*            track, 
                                                                               ...
                                                                               callingProcess)
{
  ...
  std::cout << "####################################################" << std::endl;
  std::cout << "####################################################" << std::endl;
  std::cout << callingProcess->GetWrappedProcess()->GetProcessName() << std::endl;
  std::cout << callingProcess->GetWrappedProcess()->GetProcessSubType() << std::endl;
  std::cout << "####################################################" << std::endl;
  std::cout << "####################################################" << std::endl;
  std::cout << "####################################################" << std::endl;
  std::cout << "####################################################" << std::endl;
  std::cout << "####################################################" << std::endl;
  // if(callingProcess->GetWrappedProcess()->GetProcessName() != "conv"){return 0;}
  ...
  G4double XStransformation = 20000.0 ;
      ...
      if (  previousOperation != operation )
        {
          ...
          G4Exception("OptrChangeCrossSection::ProposeOccurenceBiasingOperation(...)",
                      "exGB01.02",
                      JustWarning,
                      ed);
          return 0;
