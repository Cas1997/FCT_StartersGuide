...
void photonSpectrum(TString jobDirectory, TString saveDirectory, Int_t nJobs = 100){
	...
	for(int event = 0; event < nFCTEvents; event++){
		...
		for(Int_t iHit = 0; iHit < nFCTHits; iHit++){ // Apply other cuts
			...
			double thisRecEta = calcEta(thisX, thisY, thisZ);
			double thisRecPT = 1000. * calcRecPT(thisRecEta, thisE);

			// Smear the pT before filling the map
			double smearedRecpT = smearPT(thisRecPT, smearFactor);
			...
			// std::cout << "Filling " << thisProdProcess << " with: " << smearedRecpT << " MeV/c photon" << std::endl;
		}
	}
	...
	TString pdfFileName = saveDirectory + "/photonSpectrum_zoomOut.pdf";
	c->Print(pdfFileName);
	c->Close();
	delete c;

	// Save
	TString rootFileName = saveDirectory + "/photonSpectrum_zoomOut.root";
