...
void photSpectrumClosestDistance(TString jobDirectory, TString saveDirectory, Int_t nJobs = 100){

	Double_t paCutVal = 0.025;
	Double_t smearFactor = 0.15;
	Float_t firstLayerZ = 442.; // cm
	TString outputFileName = "closestDistance";
	...
	for(Int_t event = 0; event < nFCTEvents; event++){
		...
		for(Int_t iHit = 0; iHit < nFCTHits; ++iHit){
			...
			if(thisRecPT > 10){continue;} // To keep it in the range of interest
			...
			photonSpectra[photonSpectraIndex[thisProdProcess]]->Fill(closestDistance);
		}
	}
	...
	photonSpectra[0]->GetXaxis()->SetTitle("Closest distance (cm)");
	photonSpectra[0]->GetYaxis()->SetTitle("1/N_{evt} dN/d(c.d.) (cm)^{-1}");
