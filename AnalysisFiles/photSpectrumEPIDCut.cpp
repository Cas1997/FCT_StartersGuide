...
void photSpectrumEPIDCut(TString jobDirectory, TString saveDirectory, Int_t nJobs = 100){

	Double_t paCutVal = 0.025; // rad
	Double_t smearFactor = 0.15;
	Float_t firstLayerZ = 442.; // cm
	Float_t lastLayerZ = 500; // cm
	TString outputFileName = "photonSpectrumEPIDCut";
	...
	for(Int_t event = 0; event < nFCTEvents; event++){
		...
		// Check if an electron is present in the last layer of the FCT
		...
		for(Int_t iHit = 0; iHit < nFCTHits; iHit++){
            ...
            if(thisPID == 11 || thisPID == -11){
                if(lastLayerZ - 0.5 < thisZ && lastLayerZ + 0.5 > thisZ){
		...
		for(Int_t iHit = 0; iHit < nFCTHits; ++iHit){
            ...
            if(!(std::find(allPPP.begin(), allPPP.end(), thisProdProcess) != allPPP.end())){
				...
				if(thisProdProcess == 201){
					...
					photonSpectra.push_back(new TH1D("Low photon", "", 100, 0, 10));
					photonSpectra.back()->SetLineColor(prodNames.colorMap["Low photon"]);
					legend->AddEntry(photonSpectra.back(), "Low photon", "l");
				} else {
					photonSpectraIndex[thisProdProcess] = photonSpectra.size();
					photonSpectra.push_back(new TH1D(prodNames.TMCProcessName[thisProdProcess], "", 100, 0, 10));
