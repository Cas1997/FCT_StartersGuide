...
void Alice3Pipe::ConstructGeometry()
{
  ...
  // Window
  Double_t windowLength = 150;
  Double_t pipeASideLength = mA3IPLength / 2. - mVacuumVesselASideLength / 2. - windowLength;
  ...
  TGeoTube* pipeASide = new TGeoTube("PIPE_Ash", mPipeRIn, mPipeRIn + mPipeThick, pipeASideLength / 2.);
  TGeoTube* pipeCSide = new TGeoTube("PIPE_Csh", mVacuumVesselRIn, mVacuumVesselRIn + mVacuumVesselThick, pipeCSideLength / 2.);
  TGeoCone* window = new TGeoCone("WINDOW", windowLength/2., mVacuumVesselRIn, mVacuumVesselRIn + mVacuumVesselThick, mPipeRIn, mPipeRIn + mPipeThick);

  // Pipe and vacuum vessel positions
  TGeoTranslation* posPipeASide = new TGeoTranslation("PIPE_ASIDE_POSITION", 0, 0, mVacuumVesselASideLength / 2. + pipeASideLength / 2. + windowLength);
  posPipeASide->RegisterYourself();
  TGeoTranslation* posPipeCSide = new TGeoTranslation("PIPE_CSIDE_POSITION", 0, 0, mVacuumVesselASideLength / 2. - pipeCSideLength / 2.);
  posPipeCSide->RegisterYourself();
  TGeoTranslation* posWindow = new TGeoTranslation("WINDOW_POSITION", 0, 0, mVacuumVesselASideLength / 2. + windowLength/2.);
  posWindow->RegisterYourself();
